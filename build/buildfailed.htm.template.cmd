...
echo   ^</head^> >> %MailBody%
echo   ^<body^> >> %MailBody%
echo     ^<p^>The build ^<b class="Failed"^>FAILED.^</b^>^</p^> >> %MailBody%
echo	 ^<table^> >> %MailBody%
echo	   ^<tbody^> >> %MailBody%
echo	     ^<tr^> >> %MailBody%
...
echo       ^</tbody^> >> %MailBody%
echo	 ^</table^> >> %MailBody%
echo     ^<p^>See ^<a href="%DropLocation%/build.log"^>build.log^</a^> or the attached files for more details.^</p^> >> %MailBody%
echo   ^</body^> >> %MailBody%
echo ^</html^> >> %MailBody% 
