...
namespace ForerunnerWebService
{
    ...
    public class License
    {
        ...
        {
            string value = "<License><SKU>{0}</SKU><Quantity>{1}</Quantity><MachineKey>{2}</MachineKey><LicenseKey>{3}</LicenseKey><RequireValidation>{4}</RequireValidation><License>";
            return EncryptUsingPrivate(string.Format(value, SKU, Quantity, MachineKey, LicenseKey, RequireValidation));
        }
        ...
        private string ProcessActivate()
        {
            ...
            string SQL = @"UPDATE License Set ActivationAttempts = ActivationAttempts+1 WHERE LicenseID = @LicenseID
                           SELECT MachineKey,LastActivateDate,Quantity,l.SKU,FirstActivationDate,AllowUpgrade,Duration,RequireValidation FROM License l INNER JOIN SKU s ON l.SKU = s.SKU  WHERE LicenseID = @LicenseID";
           try
            {
                SQLConn.Open();
                ...
                while (SQLReader.Read())
                {
                    ...
                    AllowUpgrade = SQLReader.GetInt32(5);
                    LicenseDuration = SQLReader.GetInt32(6);
                    RequireValidation = SQLReader.GetInt32(7);
                }
                SQLReader.Close();
                SQLConn.Close();
                if (MachineKey == null)
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if ((StoredMachineKey == MachineKey) && (LicenseDuration > ts.TotalDays))
                        {
                            Response = String.Format(Response, "Success", "0", GetActivatePackage());
                            success = true;
                        }
        ...
        private string ProcessValidate()
        {
            ...
            string SQL = @"SELECT MachineKey,LastActivateDate,Quantity,l.SKU,FirstActivationDate,AllowUpgrade,Duration FROM License l INNER JOIN SKU s ON l.SKU = s.SKU  WHERE LicenseID = @LicenseID AND MachineKey = @MachineKey";
            ...
            try
            {
                ...
                {
                    TimeSpan ts = DateTime.Now - FirstActivationDate;
                    if (LicenseDuration < ts.TotalDays )
